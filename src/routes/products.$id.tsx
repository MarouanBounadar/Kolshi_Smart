...
// مكوّن مخصص للـ NotFound لتفادي مشاكل الـ TypeScript
...
export const Route = createFileRoute("/products/$id")({
  ...
  // حل مشكلة الـ Type عبر استخدام المكون مباشرة هنا أو تركه للموجه
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      ...
      try {
        ...
        // جلب المنتجات ذات الصلة بنفس التنسيق مع تمرير الـ stock
        ...
        if (relatedData) {
          setRelated(relatedData.map((p: any) => ({
            ...
            category: p.category,
            image: p.image, 
            stock: p.stock ?? 0, // تمرير الـ stock لبطاقة المنتجات المقترحة
            rating: p.rating || 5,
            reviews: p.reviews || 8
          })));
        }
            ...
            <div className="flex items-center gap-4 pt-2">
              <div className="flex items-center border border-white/10 bg-[#111115] rounded-2xl overflow-hidden shadow-inner">
                <button 
                  ...
                  className="px-5 py-3.5 hover:bg-white/5 text-muted-foreground hover:text-white transition text-lg font-bold cursor-pointer" 
                ...
                <button 
                  onClick={() => setQty((q) => product.stock > 0 ? Math.min(product.stock, q + 1) : 1)} 
                  className="px-5 py-3.5 hover:bg-white/5 text-muted-foreground hover:text-white transition text-lg font-bold cursor-pointer" 
